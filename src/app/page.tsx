...
export default function PublicLeaderboardPage() {
  ...
  const { data } = useSWR<{ leaderboard: Row[] }>("/api/leaderboard", fetcher, {
  ...
  return (
    <main className="min-h-screen">
      <header className="flex items-center justify-between px-6 md:px-10 py-5">
        <h1 className="text-2xl md:text-3xl font-black text-primary">營隊即時排行榜</h1>
          ...
          <ul>
            {rows.map((row) => (
              ...
              >
                ...
                <div className="w-24 text-right text-lg font-black text-primary">
                  {row.score}
                </div>
              </li>
            ))}
