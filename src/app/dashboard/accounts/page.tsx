...
export default function AccountsPage() {
  ...
  async function resetPassword(id: string) {
    const newPassword = window.prompt("請輸入這個帳號的新密碼：");
      ...
      <div className="card p-5 mb-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-black">
            批次上傳 Excel（欄位：姓名、帳號、密碼，各身分欄位填 1 或 0：管理員／老師／實驗助理／隊輔／值星官／總值星／副總值星）
          </h2>
          <a href="/api/accounts/template" className="btn-outline whitespace-nowrap text-sm">
            下載範本
          </a>
        </div>
            ...
            {accounts.map((a: any) => (
              <tr key={a.id} className="border-b border-ink/5 last:border-b-0">
                <td className="p-3">
                  <input
                    defaultValue={a.name}
                    onBlur={(e) => e.target.value !== a.name && updateField(a.id, "name", e.target.value)}
                    className="rounded-lg border border-ink/10 px-2 py-1 w-24 font-bold"
                  />
                </td>
                <td className="p-3">
                  <input
                    ...
                    onBlur={(e) =>
                      e.target.value !== a.username && updateField(a.id, "username", e.target.value)
                    }
                    className="rounded-lg border border-ink/10 px-2 py-1 w-24 text-ink/60"
                  />
                </td>
                <td className="p-3">
                  <button
                    onClick={() => resetPassword(a.id)}
                    className="text-xs btn-outline !px-2 !py-1 whitespace-nowrap"
                  >
                    重設密碼
                  </button>
                </td>
