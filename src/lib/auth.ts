...
export function availableActingRoles(session: SessionPayload) {
  ...
  if (session.isAssistant) roles.push({ key: "ASSISTANT", label: "實驗助理" });
  if (session.isCounselor) roles.push({ key: "COUNSELOR", label: "隊輔" });
  if (session.isOfficer) roles.push({ key: "OFFICER", label: "值星官" });
  if (session.isChiefOfficer) roles.push({ key: "CHIEF_OFFICER", label: "總值星" });
  if (session.isDeputyChiefOfficer) roles.push({ key: "DEPUTY_CHIEF_OFFICER", label: "副總值星" });
  return roles;
}
...
export function isSelfOnlyRole(session: SessionPayload) {
  return (
    (session.isAssistant || session.isCounselor) &&
    !session.isAdmin &&
    !session.isTeacher &&
    !session.isOfficer &&
    !session.isChiefOfficer &&
